...
public class Inventario implements InterfaceInventario{
	...
	/* Atributos
	 * 			instancia � uma instancia nula da classe inventario
	 * 			mochila � um vector para armazenar itens
	 */
	...
	public void remover_item(String nome_item){
		...
		if(item.getName() != null){
			if(mochila.contains(item.getName())){
				...
				System.out.println("N�o foi poss�vel encontrar o item " + item.getName() + " na mochila");
			}
		}else{
			System.out.println("O item " + nome_item + " n�o existe");
		}
	}
	/* Verifica se o item cont�m na mochila */
