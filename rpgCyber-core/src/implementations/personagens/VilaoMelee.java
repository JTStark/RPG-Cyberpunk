...
public class VilaoMelee extends AbsPersonagem{
	...
	public VilaoMelee(int Arma, int Armadura, int level, int nGerador){
		...
		this.forca = (int)(25 + 3 * (level / 2));
		this.percepcao = (int)(15 + 3 * (level / 5));
		this.resistencia = (int)(40 + 3 * (level/2));
		this.carisma = 0;
		this.inteligencia = (int)(25 + 3 * (level / 5));
		this.agilidade = (int)(10 + 3 * (level / 5));
		...
		this.maxHP += level * ((this.resistencia * 2)/3);
		this.hp = this.maxHP;
	}
